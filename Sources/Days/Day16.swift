...
//    }
  ...
  func part2() -> Any {
    ...
    while let path = pathsToCheck.popFirst() {
      ...
      let currentPrice = prices[path.currentPosition.position, default: Int.max]
      // We're deducting thousand to allow for multiple paths temporarily diverging and getting more expsensive,
      // until they converge to reach the destination.
      guard (path.price - 1000) <= currentPrice else {
        continue
      }
